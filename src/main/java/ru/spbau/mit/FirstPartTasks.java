...
public final class FirstPartTasks {
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.<Album>comparingDouble(
                a -> a.getTracks().stream().mapToInt(Track::getRating).average().orElse(0)).reversed()
    ...
    public static <R> Stream<R> filterIsInstance(Stream<?> s, Class<R> clazz) {
        return s.filter(cl -> clazz.isAssignableFrom(cl.getClass())).map(cl -> (R) cl);
    }
}
