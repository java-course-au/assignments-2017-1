...
public final class FirstPartTasks {
    static final int RATING_THRESHOLD = 95;
    ...
    public static List<Album> sortedFavorites(Stream<Album> albums) {
        return albums
            .filter(p -> p
                .getTracks()
                .stream()
                .anyMatch(v -> v.getRating() > RATING_THRESHOLD))
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums
            .sorted(Comparator.<Album>comparingDouble(p -> p
                    .getTracks()
                    .stream()
                    .mapToLong(Track::getRating)
                    .sum() / p.getTracks().size())
                .reversed())
            .collect(Collectors.toList());
    }
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream.reduce(1, (u, v) -> (u * v) % modulo);
    }
